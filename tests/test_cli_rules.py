...
def test_validate_invalid_config_bad_key(invalid_config_bad_key: Path):
    ...
    assert (
        "❌ Invalid config: Rule 'images' (type: extension) has unexpected keys: patern"
        in result.stdout
    )
...
def test_validate_invalid_config_missing_pattern(invalid_config_missing_pattern: Path):
    result = runner.invoke(
        app, ["rules", "validate", "--config", str(invalid_config_missing_pattern)]
    )
    assert result.exit_code == 1
...
def test_explain_rule(valid_config: Path, file_to_explain: Path):
    result = runner.invoke(
        app, ["rules", "explain", "--config", str(valid_config), "--file", str(file_to_explain)]
    )
    assert result.exit_code == 0
...
def test_explain_rule_no_match(valid_config: Path, tmp_path: Path):
    ...
    file_path.touch()
    result = runner.invoke(
        app, ["rules", "explain", "--config", str(valid_config), "--file", str(file_path)]
    )
    assert result.exit_code == 0
